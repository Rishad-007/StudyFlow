...
export function TodaysPlans({ plans }: TodaysPlansProps) {
    ...
    return (
      <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
        ...
        <div className="flex flex-col items-center py-4 text-center">
          ...
          <Link to="/plan" className="mt-2 text-sm font-medium text-indigo-600 hover:text-indigo-700">
            Create one in Plan
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
      <div className="mb-3 flex items-center justify-between">
        <h3 className="text-base font-semibold text-gray-900">Today's Plan</h3>
        <span className="text-sm text-gray-500">{done}/{plans.length}</span>
      </div>
      ...
      <div className="space-y-2">
        {plans.map((plan) => {
          ...
          return (
            <div key={plan.id} className="flex items-center gap-3 rounded-lg px-3 py-2 hover:bg-gray-50">
              <div
