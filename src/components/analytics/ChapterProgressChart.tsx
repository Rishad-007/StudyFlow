...
interface ChapterProgressChartProps {
  data: { chapter: string; subject: string; progress: number }[]
  subjectColors: Record<string, string>
}

export function ChapterProgressChart({ data, subjectColors }: ChapterProgressChartProps) {
  if (data.length === 0) {
    return (
        ...
        >
          ...
          <Bar dataKey="progress" radius={[0, 4, 4, 0]} maxBarSize={16}>
            {sorted.map((entry, i) => (
              <Cell
                key={i}
                fill={subjectColors[entry.subject] ?? '#6366f1'}
              />
            ))}
          </Bar>
        </BarChart>
